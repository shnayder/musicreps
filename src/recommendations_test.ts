...
import { buildRecommendationText } from './skill-ui-state.ts';
import { createAdaptiveSelector, createMemoryStorage } from './adaptive.ts';
import type { ItemStats, LevelStats } from './types.ts';
import { GUITAR } from './music-data.ts';
import {
  getAllLevelIds,
  getItemIdsForLevel as guitarGetItemIds,
} from './skills/fretboard/logic.ts';

// ---------------------------------------------------------------------------
// Test helpers
// ---------------------------------------------------------------------------

/** Shorthand to build a LevelStats. */
function rec(
  levelId: string,
  working: number,
  unseen: number,
...
describe('classifyLevels', () => {
  it('splits started from unstarted', () => {
    ...
    assert.equal(unstarted[0].levelId, '1');
  });

  it('returns empty arrays for empty input', () => {
    const { started, unstarted } = classifyLevels([]);
...
describe('classifyLevel', () => {
  ...
  it('detects review soon when stability is low and time has passed', () => {
    ...
    const status = classifyLevel(sel, '0', () => ['a']);
...
describe('computeLevelRecs', () => {
  function status(
    levelId: string,
    speedLabel: LevelClassification['speedLabel'],
  ...
  it('review and practice interleaved by level order, automate after', () => {
    const solidNoData: LevelClassification = {
      levelId: '2',
      speed: 0.75,
      speedLabel: 'solid',
      reviewStatus: null,
...
describe('shouldThrottleExpansion', () => {
  function status(
    label: LevelClassification['speedLabel'],
  ): LevelClassification {
    return {
      levelId: '0',
      speed: 0.75,
...
describe('computeRecommendations', () => {
  ...
  it('uses sortUnstarted to pick expansion target', () => {
    ...
    const result = computeRecommendations(
      ...
      makeGetItemIds(data),
      config,
      { sortUnstarted: (a, b) => a.levelId.localeCompare(b.levelId) },
    );
    assert.ok(result.recommended.has('1'));
    assert.equal(result.expandIndex, '1');
  });
  ...
  it('budget caps levelRecs to match recommended set', () => {
    ...
    for (const rec of result.levelRecs) {
      assert.ok(
        result.recommended.has(rec.levelId),
        `levelRec for group ${rec.levelId} not in recommended set`,
      );
    }
  });
  ...
  it('skipped level is excluded from recommendations', () => {
    const data = {
      '0': {
        workingCount: 5,
        unseenCount: 0,
  ...
  it('expansion skips past skipped level', () => {
    const data = {
      '0': {
        workingCount: 0,
        unseenCount: 0,
  ...
  it('solid + fresh level gets no rec (practiced enough)', () => {
    ...
    const ls = result.levelStatuses?.find((s) => s.levelId === '0');
    assert.ok(ls);
  ...
  it('unskipped level reappears in recommendations', () => {
    const data = {
      '0': {
        workingCount: 0,
        unseenCount: 0,
    ...
    const sort = {
      sortUnstarted: (a: LevelStats, b: LevelStats) =>
...
const ALL_GUITAR_GROUP_IDS = getAllLevelIds(GUITAR);
...
function getItemIds(levelId: string): string[] {
  return guitarGetItemIds(GUITAR, levelId);
}
...
function recText(
  ...
  const rec = computeRecommendations(
    ...
    {
      sortUnstarted: (a, b) =>
        ALL_GUITAR_GROUP_IDS.indexOf(a.levelId) -
        ALL_GUITAR_GROUP_IDS.indexOf(b.levelId),
    },
  );
