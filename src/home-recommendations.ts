...
export function computeSkillRecommendation(
  ...
  const { seen } = selector.getLevelAutomaticity(allItemIds);
...
function countStaleGroups(
  ...
  const startedIndices = allIndices.filter((idx) => {
    ...
    return ids.some((id) => selector.getAutomaticity(id) !== null);
  });

  if (!selector.getSpeedScore || !selector.getFreshness) return 0;
