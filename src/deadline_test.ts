...
describe("computeInitialDeadline", () => {
  ...
  it("clamps to minDeadline when ewma is very low", () => {
    ...
    const expectedMin = Math.round(DEFAULT_CONFIG.minTime * dlCfg.minDeadlineMargin);
  ...
  it("works with scaled config (motor baseline)", () => {
    ...
    const scaledCfg = { ...DEFAULT_CONFIG, minTime: 1500, maxResponseTime: 13500 };
...
describe("adjustDeadline", () => {
  ...
  it("does not go below minimum", () => {
    const minDeadline = Math.round(DEFAULT_CONFIG.minTime * dlCfg.minDeadlineMargin);
  ...
  it("does not go above maximum", () => {
    const result = adjustDeadline(DEFAULT_CONFIG.maxResponseTime, false, DEFAULT_CONFIG, dlCfg);
