...
export function allItems(instrument: 'guitar' | 'ukulele'): string[] {
  const out: string[] = [];
  for (const q of ALL_LEVEL_IDS) {
    out.push(...getItemIdsForLevel(instrument, q));
  }
  return out;
}
...
export const ALL_LEVEL_IDS: string[] = QUALITY_LEVELS.map((g) => g.id);
...
export function getItemIdsForLevel(
  ...
  const group = QUALITY_LEVELS.find((g) => g.id === levelId);
...
export function formatGroupLabel(enabledGroups: ReadonlySet<string>): string {
  if (enabledGroups.size === ALL_LEVEL_IDS.length) return 'all chords';
