...
describe('fretboard groups', () => {
  ...
  it('same groups for guitar and ukulele', () => {
    assert.deepEqual(getLevels(GUITAR), getLevels(UKULELE));
  });
});

// ---------------------------------------------------------------------------
// Guitar getItemIdsForLevel
// ---------------------------------------------------------------------------

describe('guitar getItemIdsForLevel', () => {
  it('frets 0-3 naturals: all items have frets 0-3', () => {
    const items = getItemIdsForLevel(GUITAR, 'frets-0-3-natural');
  ...
  it('frets 4-8 naturals: all items have frets 4-8', () => {
    const items = getItemIdsForLevel(GUITAR, 'frets-4-8-natural');
    assert.ok(items.length > 0);
    for (const id of items) {
      const fret = Number(id.split('-')[1]);
  ...
  it('frets 9-12 naturals: all items have frets 9-12', () => {
    const items = getItemIdsForLevel(GUITAR, 'frets-9-12-natural');
    assert.ok(items.length > 0);
    for (const id of items) {
      const fret = Number(id.split('-')[1]);
  ...
  it('all groups cover all 78 items', () => {
    ...
    for (const id of getAllLevelIds(GUITAR)) {
      for (const itemId of getItemIdsForLevel(GUITAR, id)) {
...
describe('ukulele getItemIdsForLevel', () => {
  it('all groups cover all 52 items', () => {
    const allFromGroups = new Set<string>();
    for (const id of getAllLevelIds(UKULELE)) {
      for (const itemId of getItemIdsForLevel(UKULELE, id)) {
        allFromGroups.add(itemId);
      }
    }
...
describe('formatLabel', () => {
  ...
  it('all groups', () => {
    assert.equal(
      formatLabel(GUITAR, new Set(getAllLevelIds(GUITAR))),
      'all groups',
    );
  });
