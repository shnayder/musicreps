...
const LEVEL_LONG_LABELS: Record<number, string> = {
  0: 'Major triads',
  1: 'Minor triads',
  2: 'Dominant 7th',
  3: 'Major 7th',
...
const LEVEL_IDS: Record<number, string> = {
  0: 'triads-major',
  1: 'triads-minor',
  2: 'dom-7th',
  3: 'maj-7th',
...
function buildGroups(): {
  ...
  for (let g = 0; g <= maxGroup; g++) {
    ...
    const longLabel = LEVEL_LONG_LABELS[g] ?? label;
    const id = LEVEL_IDS[g] ?? String(g);
...
export const SPELLING_LEVELS = buildGroups();
...
export function getItemIdsForLevel(levelId: string): string[] {
  const group = SPELLING_LEVELS.find((g) => g.id === levelId);
  if (!group) return [];
  const items: string[] = [];
...
export const ALL_LEVEL_IDS: string[] = SPELLING_LEVELS.map((g) => g.id);
