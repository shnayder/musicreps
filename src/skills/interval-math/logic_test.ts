...
describe('DISTANCE_LEVELS', () => {
  it('has 5 groups', () => {
    assert.equal(DISTANCE_LEVELS.length, 5);
  });

  it('each group has distances and label', () => {
    for (const group of DISTANCE_LEVELS) {
  ...
  it('groups cover semitone distances 1–11', () => {
    const allDistances = DISTANCE_LEVELS.flatMap((g) => g.distances);
  ...
  it('first group contains distances 1 and 2 (m2 and M2)', () => {
    assert.deepEqual(DISTANCE_LEVELS[0].distances, [1, 2]);
  });

  it('last group contains distances 9, 10, and 11 (M6 m7 M7)', () => {
    const last = DISTANCE_LEVELS[DISTANCE_LEVELS.length - 1];
...
describe('getItemIdsForLevel', () => {
  it('group 0 contains items with m2 and M2', () => {
    const ids = getItemIdsForLevel('seconds');
  ...
  it('group 4 contains M6, m7, and M7 items', () => {
    const ids = getItemIdsForLevel('sixths-sevenths');
