...
const scenarios: Scenario[] = [
  ...
  { label: "Wrong (high stability)", oldS: 96, resp: null, elapsed: null, wrong: true },
  { label: "Wrong (medium stability)", oldS: 16, resp: null, elapsed: null, wrong: true },
  { label: "Wrong (low stability)", oldS: 10, resp: null, elapsed: null, wrong: true },
];

const s2Headers = ["Scenario", "oldS", "resp", "elapsed", "newS", "newS(days)", "growth"];
...
const s2Rows = scenarios.map(sc => {
  ...
  const growth = sc.oldS != null && sc.oldS > 0 ? (newS / sc.oldS).toFixed(2) + "x" : "-";
  return [
...
const weightScenarios: WeightScenario[] = [
  { label: "Fast (1000ms), just answered", ewma: 1000, stability: 4, lastCorrectAt: now },
  { label: "Medium (2500ms), just answered", ewma: 2500, stability: 4, lastCorrectAt: now },
  { label: "Slow (5000ms), just answered", ewma: 5000, stability: 4, lastCorrectAt: now },
  { label: "Fast (1000ms), recall~0.8", ewma: 1000, stability: 4, lastCorrectAt: now - 4 * 0.32 * 3600000 },
  { label: "Fast (1000ms), recall~0.5", ewma: 1000, stability: 4, lastCorrectAt: now - 4 * 3600000 },
  { label: "Fast (1000ms), recall~0.3", ewma: 1000, stability: 4, lastCorrectAt: now - 4 * 1.74 * 3600000 },
  { label: "Slow (5000ms), recall~0.5", ewma: 5000, stability: 4, lastCorrectAt: now - 4 * 3600000 },
  { label: "Slow (5000ms), recall~0.3", ewma: 5000, stability: 4, lastCorrectAt: now - 4 * 1.74 * 3600000 },
  { label: "Fast (1000ms), no stability data", ewma: 1000, stability: null, lastCorrectAt: null },
  { label: "Slow (5000ms), no stability data", ewma: 5000, stability: null, lastCorrectAt: null },
];

const w3Headers = ["Item state", "speedW", "recall", "recallW", "total", "vs unseen"];
...
type Trajectory = { label: string; interval: number; responseTime: number; sessions: number };

const trajectories: Trajectory[] = [
  { label: "Daily, fast (1200ms)", interval: 24, responseTime: 1200, sessions: 10 },
  { label: "Daily, medium (3000ms)", interval: 24, responseTime: 3000, sessions: 10 },
  { label: "Daily, slow (6000ms)", interval: 24, responseTime: 6000, sessions: 10 },
  { label: "Every-other-day, fast", interval: 48, responseTime: 1200, sessions: 10 },
];

for (const traj of trajectories) {
  console.log(`  ${traj.label} (${traj.sessions} sessions, every ${fmtHours(traj.interval)}):`);
  ...
  for (const n of sessionNums) {
    ...
    const recall = elapsed != null ? computeRecall(stability, traj.interval) : null;
