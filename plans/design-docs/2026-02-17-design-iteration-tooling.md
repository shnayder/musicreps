...
`modeScreen()`, `fretboardSVG()`, and button helpers as the production app.
This ensures it never drifts from reality. Run `npx tsx build.ts` after CSS
edits to regenerate.
...
Each section is **generated by `buildMoments()` in `build.ts`**, which calls
the real `modeScreen()` helpers and then applies `prepareMoment()` string
