...
Implements the [timer design spec](../product-specs/active/2026-02-13-spaced-repetition-timer-spec.md).
...
Pure functions + factory, same pattern as `adaptive.js`. ES module so tests
can import it.
...
**Storage:** Extend the existing adaptive storage adapter — add a parallel
key namespace (`deadline_{namespace}_{itemId}`) so deadlines sit alongside
adaptive stats in localStorage.
...
Add `timedOut: boolean` to engine state. The `engineSubmitAnswer` function
gains a variant for timeouts:
...
4. **`submitAnswer()` integrates with deadline tracker**: after checking
   answer, calls `deadlineTracker.recordOutcome(itemId, correct)`.

5. **Deadline display**: Show deadline value next to the countdown bar.
   Add `els.deadlineDisplay` DOM reference. Updated in `nextQuestion()`.
...
3. **Add `engineTimedOut` state transition** to `quiz-engine-state.js`.
   Modify `engineSubmitAnswer` to format time as "X.Xs" instead of "X ms".
...
7. **Update `main.ts` and `build.ts`** — add `readModule("./src/deadline.js")`
   to the file reads and concatenation. Must be after `adaptive.js` (depends
   on storage) and before `quiz-engine.js` (consumed by engine).
...
- **engineTimedOut state**: feedback text, feedback class, timedOut flag,
  time display format
