...
Persist in `localStorage.fretboard_notation` ('letter' | 'solfege').
Load on module evaluation (before any mode initializes).
...
In modes that dynamically set button labels (semitone math, interval math),
the `presentQuestion()` already sets labels — those call sites wrap in
...
- Reverse prompt: `${displayNote(rootNote)}${qual} in ${displayNote(key.root)} major = ?`
