...
Speed Tap records total round time (finding all 6-8 positions of a note),
but both heatmap modes use single-tap thresholds:
...
- **Retention heatmap**: `computeSpeedScore` uses `automaticityTarget = 3000ms`,
  so a 6s EWMA yields speedScore ≈ 0.06, making automaticity near-zero even
  with perfect recall.
...
In `showNoteStats`, for speed mode, normalize EWMA to per-position time
before passing to `getSpeedHeatmapColor`:
...
This reuses the existing color scale (green < 1.5s/pos, etc.) without
changing the shared function signature. ~6s for 7 positions = 0.86s/pos → green.
