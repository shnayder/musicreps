...
- `guides/design/moments.html` — **generated by `buildMoments()` in `main.ts`**.
  Shows assembled screen layouts at 402px using the same `modeScreen()` and
  button helpers as production. To add or modify a moment, edit `buildMoments()`.
